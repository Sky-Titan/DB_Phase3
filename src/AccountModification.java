...
public class AccountModification extends JFrame{
	...
	private JLabel pw_label, pw_confirm_label, kind_label, fname_label, lname_label, phone_label;//필수 입력
	private JLabel bdate_label, gender_label, job_label, country_label, city_label, detail_address_label;//추가 입력
	private JTextField fname, lname, phone, job, country, city, detail_address;
	...
	public AccountModification(String id) {
		super("회원 정보 수정 "+id);
		this.id = id;
		...
		pw_confirm.setText(accountInfo[4]);
	...
	private class Handler implements ActionListener
	{
		...
		{
			if(event.getSource()==complete)//회원정보수정
			{
				...
				if(!pw.getText().equals("") && !pw_confirm.getText().equals("") && !fname.getText().equals("") 
